...
function Header(doc, Folio) {
  ...
  doc.textWithLink("Contrato de Prestación", 210, 45, {
  ...
  try {
    ...
    doc.textWithLink("Miembro Activo", 385, 85, {
      url: "https://goo.gl/maps/KgRQhTxJQoMNe8nv9",
    });
...
function Detalles(
  ...
  splitObservacion = doc.splitTextToSize(
  ...
  try {
    ...
    doc.text(
      120,
      starty,
      IMPORTE_TOTAL.replace("$", "").replace(",", "").trim()
    );
  }
  ...
  try {
    doc.text(
      ...
      ANTICIPO +
        " " +
        miConversor.convertToText(
          ANTICIPO.replace("$", "").replace(",", "").replace(".00", "").trim()
        )
    );
  } catch (error) {
  ...
  try {
    doc.text(
      ...
      SALDO +
        " " +
        miConversor.convertToText(
          SALDO.replace("$", "").replace(",", "").replace(".00", "").trim()
        )
    );
  } catch (error) {
  ...
  var derivado = doc.splitTextToSize(
    "Derivado del Servicio de Transporte Terrestre que solicite a la empresa GVA Tours S.A. de C.V., y una vez que he leido el total de las condiciones que se adjuntan al presente contrato, estoy de acuerdo con el total de su contenido.",
    530
  );
  ...
  doc.text(
    150,
    starty,
    "TELEFONO DE EMERGENCIA: GUSTAVO JAUREGUI 333-808-6093"
  );
...
function condiciones(doc, autorizador) {
  ...
  texto =
    "1.- Los pasajeros se comprometen a respetar los horarios e itinerarios estipulados tanto de salidas como de llegadas, de no ser así se le cobrará al contratante el tiempo excedente proporcionalmente a lo estipulado en este contrato, por lo que en caso de ampliar la ruta, el horario o cualquier modificación al kilometraje acordado, en el presente contrato, será cubierto con una cantidad adicional fijada por la empresa transportista en la ciudad donde se celebró el contrato.";
  let splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div);

  texto =
    "2.-Cuando el contratante cancele este contrato tendra un cargo por cancelacion del 20% del Total del mismo, siempre y cuando lo haga con un minimo de 15 dias antes dela fecha y horario establecido de salida.";
  splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div) + 17;

  texto =
    "Si el aviso de cancelación se realiza entre 14 a 10 días la sanción será del 30%, si es de 9 a 5 días la sanción será del 50% y de 4 días a la fecha y hora de salida será del 100%. Esto mismo aplica para la cancelación de los servicios adicionales contratados.";
  splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div);
  texto =
    "El aviso de cancelación deberá darse por escrito y con acuse de recibido.";
  splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div) + 17;
  texto =
    "3.- Si el contratante interrumpiera su viaje en un lugar u horario intermedio de lo acordado en el presente contrato por causas no imputables a GVA Tours S.A. de C.V., el cliente NO tendrá derecho al reembolso. ";
  splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div);
  texto =
    "4.- El número de pasajeros en ningún caso deberá exceder la capacidad autorizada del vehículo que se contrate, de lo contrario GVA Tours S.A. de C.V. solo estará obligado a aceptar y realizar el viaje con la capacidad de personas previamente establecida en el contrato. Si aun así el contratante continuara exigiendo el servicio y a voluntad de GVA Tours S.A. de C.V. se realizara el mismo. El contratante se hará responsable de todas y cada una de las sanciones que la Secretaria de Comunicaciones y Transportes o cualesquiera otra autoridad aplique al respecto, sin responsabilidad para GVA Tours S.A. de C.V. ";
  splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div);
  texto =
    "5.- En el caso en que la unidad contratada no se presente en el lugar y fecha establecidos, la empresa GVA Tours S.A. de C.V., queda obligada a devolver el importe cobrado; el contratante no podrá exigir una mayor cantidad a la expresada en el presente contrato.";
  splitObservacion = doc.splitTextToSize(texto, 250);
  doc.text(40, starty, splitObservacion);
  starty += tabinsidesection * parseInt(texto.length / div);
  ...
  let columna2 =
    "6.- GVA Tours S.A. de C.V. no se hace responsable de daños o perjuicios que se provoquen al contratante por retrasos en llegada o salida de cualquiera de los puntos señalados en el contrato debido a las fallas mecánicas, condiciones desfavorables del tiempo, impedimento de pasos, revisiones oficiales y otras causas de fuerza mayor. ";
  splitObservacion = doc.splitTextToSize(columna2, 250);
  doc.text(300, starty + 13, splitObservacion);
  starty += tabinsidesection * parseInt(columna2.length / div);

  columna2 =
    "7.- Se prohíbe al contratante conjuntamente con los pasajeros ingerir bebidas alcohólicas y/o drogas durante el trayecto del viaje. La empresa se reserva el derecho de transportación de esta (s) persona (s).";
  splitObservacion = doc.splitTextToSize(columna2, 250);
  doc.text(300, starty + 13, splitObservacion);
  starty += tabinsidesection * parseInt(columna2.length / div);

  columna2 =
    "8.- Los daños ocasionados en la unidad por negligencia o culpa directa del contratante o pasajeros, serán cobrados en su totalidad al contratante.";
  splitObservacion = doc.splitTextToSize(columna2, 250);
  doc.text(300, starty + 13, splitObservacion);
  starty += tabinsidesection * parseInt(columna2.length / div);

  columna2 =
    "9.- GVA Tours S.A. de C.V. no se hace responsable por objetos olvidados en la unidad por alguno de los pasajeros.";
  splitObservacion = doc.splitTextToSize(columna2, 250);
  doc.text(300, starty + 13, splitObservacion);
  starty += tabinsidesection * parseInt(columna2.length / div);

  columna2 =
    "10.-  Las unidades no pueden entrar en caminos estrechos o de terracería que pongan en peligro a la unidad. Cualquier daño o perjuicio por insistencia del cliente será cobrada en su totalidad.";
  splitObservacion = doc.splitTextToSize(columna2, 250);
  doc.text(300, starty + 13, splitObservacion);
  starty += tabinsidesection * parseInt(columna2.length / div);
  ...
  columna2 =
    "12.- GVA Tours S.A. de C.V. se obliga en caso de descompostura reemplazar la unidad con características semejantes a la contratada, en un plazo de tiempo igual al tiempo de recorrido del punto de descompostura a la base de la empresa en la ciudad de Guadalajara.";
...
function pieCharter(doc) {
  ...
  starty += 10;
  let leyenda0 =
    "Charter Terrestre es un servicio de Transporte Turístico, el cual debe de estar acompañado de una Reservación de Hotel;";
  doc.text(leyenda0, xcenter(doc, leyenda0), starty);

  starty += 10;
  leyenda0 =
    "por eso es obligatorio presentar su Cupón o confirmación de Hospedaje al abordar la unidad en el punto de salida;";
  doc.text(leyenda0, xcenter(doc, leyenda0), starty);
  ...
  starty += 10;
  leyenda0 =
    "Cualquier cambio de fecha aplica un cargo de $200 por persona adulto y niños sujeto a disponibilidad";
  doc.text(leyenda0, xcenter(doc, leyenda0), starty);
  ...
  const leyenda3 =
    "Av. Fidel Velazquez #643-A, Col. Santa Elena Alcande., CP 44230, Guadalajara, Jal. ";
